...
the entire named dependency-connected story group, remediation, or debugging
...
reviewers, or workers, work outside the assigned group, or recommend a next action.
...
after your changes are attributed. You may use approved `allowed_commands` for
local feedback while developing. Do not duplicate the full proof set just to
write a report. Preserve tests and all acceptance, regression, native contract,
runtime, and observable-value obligations. Complete all assigned stories as a
coherent unit within the approved boundary.
